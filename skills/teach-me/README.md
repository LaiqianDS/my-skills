...
3. **Proving it stuck** - the production rule and its three probes.
4. **The artifacts** - the templates, plus the three things the templates cannot say on their own.
5. **Returning to a course** - the branch for a map that already exists.
6. **Hard rules** - the constraints that keep the tutor from drifting back into being an explainer.

The example sits in `references/` rather than in `SKILL.md` because it is only needed once per course, at the moment a map gets drawn.
...
Two design choices are worth naming, because they are what keep the system small:
...
The workspace idea, the mission-first rule, and the equal-length answers rule come from the `teach` skill by [Matt Pocock](https://github.com/mattpocock).
